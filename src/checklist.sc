    ...
    state: Checklist
        ...
        state: LoopBySteps
            script:
                ...
                if ($session.stepNumber < checklist[$session.checklistType].steps.length) {
                    ...
                    // запоминаем id - сообщения и текст, на случай повторного отправления этого сообщения
            ...
            if: $session.ErrorCounter === 3 
                ...
                script:
                    ...
                    # если сообщения от пользователя нет, то переходим в стейт '/Checklist/LoopBySteps/timeout'
            ...
            state: ShiftfileEvent
                ...
                event: fileEvent
                script:
                    // если пользователь еще не отправлял файл, то сохраняем его в $client.report
        ...
        state: Report
            script:
                ...
                var dayInGenitive = checklist[$session.checklistType].dayInGenitive;
                // использование функции $nlp.inflect использует запросы к ядру Сaila и уменьшает кол-во Cailа API на 1 при срабатывании сценария в этом месте.
                //var dayInGenitive = $nlp.inflect(checklist[$session.checklistType].dayName, "gent");
                // формируем день недели в винительном падеже
                var dayInAccs = checklist[$session.checklistType].dayInAccs;
                ...
                if (interval > 420){
                    ...
                    var message = "Сотрудник " + $client.name +" "+ $client.username + " справился с чек-листом чистоты за " + dayInAccs + ". время выполнения - " + interval + " мин." + " \nОтчёт:";
                    // использование функции $caila.conform использует запросы к ядру Сaila и уменьшает кол-во Cailа API на 1 при срабатывании сценария в этом месте.
